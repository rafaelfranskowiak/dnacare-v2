...
export default function Sidebar({ links, title = 'DNA Care' }: SidebarProps) {
  ...
  return (
    <aside
      className={`flex flex-col border-r border-edge bg-surface transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      <div
        className={`flex h-16 items-center border-b border-edge ${
      ...
      <div className="border-t border-edge p-3">
        <button
          ...
          className={`flex w-full items-center rounded-lg px-3 py-2 text-ink-tertiary transition-colors hover:bg-surface-canvas hover:text-ink ${
            collapsed ? 'justify-center' : 'gap-3'
          }`}
        ...
        >
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-edge-soft bg-brand/10 text-xs font-semibold text-brand">
            {initials}
          </div>
          {!collapsed && (
            <div className="flex-1 truncate">
              <p className="text-sm font-medium text-ink">{user?.name || 'Admin'}</p>
              <p className="text-xs text-ink-tertiary">{user?.email || 'admin@app.com'}</p>
            </div>
          )}
        </div>
        ...
        <button
          onClick={logout}
          className={`mt-1 flex w-full items-center rounded-lg px-3 py-2 text-sm text-ink-tertiary transition-colors hover:bg-danger/10 hover:text-danger ${
            collapsed ? 'justify-center' : 'gap-3'
          }`}
          title="Sair"
        >
